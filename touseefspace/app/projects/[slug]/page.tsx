...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title: `${project.title} — Case Study | touseefspace`,
      description,
      url: `https://touseefspace.com/projects/${slug}`,
      type: "article",
    },
    twitter: {
...
export default async function ProjectCaseStudyPage({ params }: PageProps) {
  ...
  return (
    <article className="page-shell pt-24 pb-20 sm:pt-32 sm:pb-28 min-w-0 max-w-full">
      ...
      <nav className="mb-10 flex items-center gap-4 text-xs font-mono font-medium text-(--ink-muted)">
        <Link
          href="/projects"
          className="inline-flex items-center gap-1.5 hover:text-(--ink-primary) transition-colors group"
        >
